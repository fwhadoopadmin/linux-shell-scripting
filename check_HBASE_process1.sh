...
#--------------------------------------------------------------------
result="$(ps -ef|grep -i hbase  |grep -v "grep" |grep -i hbase-hbase | awk '{print $2}' |wc -l)"

echo $result 2>&1 >/dev/null;
if [ $result -le 0 ]; then
    echo "HBASE WARNING" 2>&1 >/dev/null;
    status="WARN"
    msg1="HBASE Warning!!: HBASE is either Down  or not in this machine please validate"
    ...
    status="OK"
    msg1="HBASE Process Running Successfully & OK: Condition=HEALTHY"
