...
result="$(ps -ef|grep -i mapreduce  |grep -v "grep" |grep -i jar | awk '{print $2}' |wc -l)"

echo $result 2>&1 >/dev/null;
if [ $result -le 0 ]; then
    echo "SWAP WARNING" 2>&1 >/dev/null;
    status="WARN"
    msg1="ATLAS Warning!!: Atlas is either Down  or not in this machine please validate"
    ...
    status="OK"
    msg1="ATLAS Process Running Successfully & OK: Condition=HEALTHY"
